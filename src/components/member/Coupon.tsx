...
import { RiContactsBookFill, RiCoupon3Fill } from "react-icons/ri";
...
import { contactBook, couponList } from "../../../public/assets/nationalities";
import ContactCard from "./ContactCard";
import CouponCard from "./CouponCard";

export default function Coupon({ show, setShow }: any) {
  const [searchState, setSearchState] = useState("");
  const [list, setList] = useState(couponList);
  const {
    logInState,
    onLogIn,
    // show,
  ...
  return (
    ...
    >
      <div className="desktop1:w-[70%] w-[90%] desktop2:w-[50%] px-10 py-10 absolute top-[12%] left-[50%] translate-x-[-50%]  rounded-xl  bg-slate-100 z-[20]">
        <div className="relative w-full h-full flex flex-col items-center justify-center gap-4">
          ...
          <div className="text-[rgb(36,120,92)] flex items-center gap-2">
            <RiCoupon3Fill size={30} />

            <h1 className="text-xl uppercase ">Coupon</h1>
          </div>
            ...
            <div className="flex justify-center  items-center w-full  desktop0:gap-2 gap-1 tablet1:gap-4 ">
              ...
              <div className=" w-full  ">
                ...
                <input
                  ...
                  type="text"
                  placeholder=". . . . . . "
                  className={` input input-bordered w-full  bg-[rgba(212,219,193,0.3)] text-slate-700`}
                />
              </div>
            </div>
          ...
          <div className="w-full flex-col item-start ">
            ...
            <div className="w-full h-[500px] overflow-scroll flex flex-col gap-[2px]">
              {list?.map((i: any, index: any) => (
                <CouponCard item={i} key={index} index={index} />
              ))}
            </div>
          </div>
        </div>
